...
class App
{
    ...
    static public function execApp() {
        //防止CSRF，排出支付异步通知
        if(APP_NAME != 'classroom' && MODULE_NAME != 'Pay' && MODULE_NAME != 'Qiniu'){
            preg_match('/http(s)?:\/\/(.*)\.(.*\..*)/',SITE_URL,$matchs);
            if(strtoupper($_SERVER['REQUEST_METHOD'])=='POST' && IS_CHILDS_HOST === 0 && stripos($_SERVER['HTTP_REFERER'],'.'.$matchs[3]) === false && $_SERVER['HTTP_USER_AGENT'] !== 'Shockwave Flash') {
                die('illegal request.');
            }
        }
...
//            }
    ...
    static public function execWidget() {
        ...
        //防止CSRF
        if(strtoupper($_SERVER['REQUEST_METHOD'])=='POST'&& IS_CHILDS_HOST === 0 && stripos($_SERVER['HTTP_REFERER'], SITE_URL)!==0 && $_SERVER['HTTP_USER_AGENT'] !== 'Shockwave Flash') {
            die('illegal request.');
        }
