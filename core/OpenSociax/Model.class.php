...
//     protected function getSourceInfo() {}
     ...
     * @return mixed
     */
     public function findPage($pageopt,$count=false,$options=array(),$record = '') {
        // dump($pageopt);
        // dump($count);
        // dump($options);
        // dump($record);
		...
		if($count===false){
            ...
            if($record == 'record'){
                $count = count($this->learnRecord($options));
            }else{
		...
		if($count > 0) {
            ...
            if($record == "record"){
                $resultSet = $this->learnRecord($options);
            }else{
    ...
    //获取当前用户的观看直播id和记录id
    public function learnRecord($options){
        
        $learn = M("learn_record")->where("userName='".$options['userName']."'")->order($options['order'])->limit($options['limit'])->select();
        // echo M()->getLastSql();
        // dump($learn);
        $array_data = [];
        foreach ($learn as $key => $val) {
            if($val['recordType'] == 2){
                if($val['action'] == 1){
                    $array_data[$val['roomId']][$val['recordId']][1][] = $val['leaveTime'];
                }elseif($val['action'] == 0){
                    $array_data[$val['roomId']][$val['recordId']][0][] = $val['enterTime'];
                }
                    $array_data[$val['roomId']][$val['recordId']]['record'] = $val;
            }else{
                $array_data[$key] = $val;
            }
        ...
        $array_new = [];
        foreach($array_data as $k1 => $v1){
                foreach($v1 as $k2 => $v2){
                    if(is_array($v2) && array_key_exists("record", $v2)){
                        foreach($v2[0] as $k3 => $v3){
                            $array_new['id'] = rand(11111,999999);
                            $array_new['userName'] = $v2['record']['userName'];
                            $array_new['roomName'] = $v2['record']['roomName'];
                            $array_new['roomId'] = $v2['record']['roomId'];
                            $array_new['liveId'] = $v2['record']['liveId'];
                            $array_new['recordId'] = $v2['record']['recordId'];
                            $array_new['ip'] = $v2['record']['ip'];
                            $array_new['enterTime'] = $v3;
                            $array_new['leaveTime'] = $v2[1][$k3];
                            $array_new['createTime'] = $v2['record']['createTime'];
                            $array_new['recordType'] = '录播';
                            $array_new['watchTime'] = gmdate("H:i:s",(strtotime($v2[1][$k3])-strtotime($v3)));
                            array_push($array_data, $array_new);
                        }
                        unset($array_data[$k1]);
                    }else{
                        if($v1['recordType'] == 1){
                            $array_data[$k1]['recordType'] = "直播";
                            $array_data[$k1]['watchTime'] = gmdate("H:i:s",$v1['watchTime']);
                        }else{
                            $array_data[$k1] = $v1;
                        }
                    }
