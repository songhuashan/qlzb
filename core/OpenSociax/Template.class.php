...
/*        if(ini_get('short_open_tag'))
    ...
    public function parse($content) {
        ...
        // 首先替换literal标签内容
        //$content = preg_replace(,"\$this->parseLiteral('\\1')",$content);
        $patterns = '/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/is';
        ...
        //解析普通模板标签 {tagName:}
        //$content = preg_replace('',"\$this->parseTag('\\2')",$content);
		$patterns = '/('.$this->config['tmpl_begin'].')(\S.+?)('.$this->config['tmpl_end'].')/is';
		$content = preg_replace_callback($patterns,function($matches){
                ...
                if(false !== stripos($content,C('TAGLIB_BEGIN').$startTag)) {
                    if(empty($tag['attribute'])){
                        ...
                        if($tag['content'] !='empty'){
							$patterns = '/'.$begin.$startTag.'(\s*?)'.$end.'(.*?)'.$begin.'\/'.$endTag.'(\s*?)'.$end.'/is';
							for($i=0;$i<$level;$i++) {
								$content = preg_replace_callback($patterns,function($matches) use ($tagLib,$tagName){
									return $this->parseXmlTag($tagLib,$tagName,$matches[1],$matches[2]);
								},$content);
							}
						}else{
							$patterns = '/'.$begin.$startTag.'(\s*?)\/(\s*?)'.$end.'/is';
							$content = preg_replace_callback($patterns,function($matches) use($tagLib,$tagName){
								return $this->parseXmlTag($tagLib,$tagName,$matches[1],'');
							},$content);
                        }
