...
*获取roomID
*/
admin.userLiveRecord = function(uid){
    var sendUrl = U('admin/User/userLiveRecord')+'&uid='+uid; 
    $("#load").show();
    $.ajax({
        type:'post',
        url : sendUrl,
...
*获取 直播记录和观看回放记录
*/
admin.userWatchList = function(uid){
    var sendUrl = U('admin/User/userWatchList')+'&uid='+uid; 
    $("#load").show();
    $.ajax({
        type:'post',
        url : sendUrl,
...
admin.delLearnAll = function(uid){
    var sendUrl = U('admin/User/delLearnAll')+'&uid='+uid; 
    $("#load").show();
    $.ajax({
        type:'post',
        url : sendUrl,
...
//签合同
admin.signContract = function(id){
    if( 'undefined' == typeof(id) || id == ''){
        id = admin.getChecked();
    }
    if(id == ''){
    ...
    $.post(U('admin/User/doSignContract'),{id:id},function(msg){
        admin.ajaxReload(msg);
    },'json');
...
admin.doaction = function(id,action,type,status){
    ...
    if(confirm( '确定操作该条记录？' )){
        $.post(U('admin/AdminSeo/doaction'+action),{id:id,type:type,status:status},function(obj){
            if(obj.status == 1){
