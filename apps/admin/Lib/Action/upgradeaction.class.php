...
class UpgradeAction extends AdministratorAction
{
    ...
    public function check()
    {
        if (version_compare(PHP_VERSION, '5.5', '<')) {
            $this->showError('您当前PHP: '.PHP_VERSION.'版本低于Eduline运行的最低版本PHP:5.5');
        }
    ...
    private function showUpgrade($log, $downUrl)
    {
        ...
        $this->pageTab [] = array ( 'title' => '在线升级', 'tabHash' => 'showUpgrade', 'url' => 'javascript:;' );
        $this->opt['tips'] = '<pre style="color:#f00;font-size:20px">
=====如果在线升级：<font color="#000">
1.升级前请做好网站程序和数据备份。
2.如果您网站经过修改，请勿使用在线升级。
3.升级需要Eduline程序文件和目录拥有可写，可读权限，升级前先确定
4.升级前一定要做好数据备份。
...
<pre/>';
    ...
    public function step1()
    {
        ...
        // # 备份老配置文件
        $oldConf = file_get_contents(CONF_PATH.'/eduline.inc.php');
        file_put_contents(DATA_PATH.'/old.eduline.inc.php', $oldConf);
    ...
    public function step3()
    {
        ...
        $delFilePath = dirname($sqlFilePath).'/deleteFiles.php';

        // # 删除废弃文件
        if (file_exists($delFilePath)) {
            $delFile = include $delFilePath;
        ...
        if (file_exists($sqlFilePath)) {
            ...
            if (isset($result['error_code'])) {
                // # 回滚配置文件
                $oldConf = file_get_contents(DATA_PATH.'/old.eduline.inc.php');
                file_put_contents(CONF_PATH.'/eduline.inc.php', $oldConf);
        ...
        // 删除版本配置备份
        @unlink(DATA_PATH.'/old.eduline.inc.php');
