...
//		$this->pageTab[] = array('title' => '赠送', 'tabHash' => 'addCourseOrder', 'url' => U('classroom/AdminCourseOrder/addCourseOrder'));
    ...
    public function index_()
    {
        ...
        $this->pageKeyList = array(
            'id', 'user_group_name', 'video_title', 'ctype', 'addtime'
        );
        //页面按钮
        ...
        $map['discount_type'] = 4;
        //取得数据列表
        $listData =M()->query("select c.id,c.ctype,c.num,c.user_group_name,c.addtime,d.video_title from ( select a.*,b.user_group_name from el_group_giving a left join el_user_group b on(a.gid=b.user_group_id) ) as c left join el_zy_video as d on(c.kid=d.id)");
    ...
    public function doCourseOrder(){
        ...
        if($_POST['pattern'] == 1){
            if(!t($_POST['uids'])){
                $this->ajaxReturn('',"请选择用户",0);
            }
            $uids = array_filter(explode(',',$_POST['uids']));
        }else if($_POST['pattern'] == 2){
            if(!t($_POST['user_group'])){
                $this->ajaxReturn('',"请选择用户组",0);
            }
		...
		if($type == 1){
			...
			foreach ($uids as $key => $val) {
				$live_pay_status = M('zy_order_live')->where(['uid'=>$val,'live_id'=>$video_id])->field('id,pay_status')->find();

				echo $live_pay_status['pay_status'];
				exit();
				if($live_pay_status['pay_status'] == 3 || $live_pay_status['pay_status'] == 6){
					unset($val);
				} else if ($live_pay_status['pay_status'] == 1 || $live_pay_status['pay_status'] == 2 || $live_pay_status['pay_status'] == 5 ||$live_pay_status['pay_status'] == 7){
					D("ZyOrderLive",'classroom')->where(array('uid'=>$val, 'id'=>$live_pay_status['id']))->save($pay_data);
        ...
        if($order_res){
            ...
            $data['kid']=$_POST['video_ids'];
            $data['gid']=$_POST['user_group'];
            $data['ctype']=$_POST['course_type'];
            $data['addtime']=time();
            $data['num']='0';
            
            $res = model('GroupGiving')->addgroup($data);
