...
$this->onload[] = "admin.departDefault('".implode(',', $_POST['_parent_dept_id'])."','form_user_department')";
        ...
        foreach ($listData['data'] as $k => $v) {
            ...
            switch (strtolower($type)) {
                ...
                case 'dellist':
                    ...
                    $listData['data'][$k]['DOACTION'] .= '<a href="' . U('admin/User/doSignContract', array('tabHash' => 'signContract', 'uid' => $v['uid'])) . '">签合同</a> - ';
    ...
    public function learn($limit = 20)
    {
        ...
        $this->pageKeyList   = array('id', 'title', 'enterTime','leaveTime','watchTime','recordType','recordId','roomId','liveId','ip','createTime');
                $this->searchKey     = array('id', 'uid', 'video_title', 'sid');
//        $video_title = M('zy_video')->where('id='.$id)->getField("video_title");
        $userName = M("user")->where("uid=".$uid)->field("phone")->find();
        $indexAction = new IndexAction();
        $userPhone = $indexAction->hidtel($userName['phone']);
        // $learn = M('learn_record')->where('uid=' . $uid)->order("ctime DESC")->findPage($limit);
        $learn = M('learn_record')->where('userName like ' . "'".$userPhone."%'")->order("createTime DESC")->findPage($limit);
        
        foreach ($learn['data'] as &$val) {
            $val['createTime'] = date('Y-m-d H:i:s', $val['createTime']);
//            $val['stime'] = date('Y-m-d H:i:s',$val['stime']) ;
            //            $val['time'] = date('Y-m-d H:i:s',$val['time']) ;
            // $liveInfo = M('el_live_record')->where(array('id' => $val['liveId']))->find();
            // $val['title']  = $liveInfo['roomName'];
            // $val['recordId'] = $liveInfo['recordId'];
    ...
    public function userWatchList()
    {
        ...
        foreach($roomIdList as $key => $val){
            ...
            if("OK" == $result_list->result && 0 < $result_list->count){
                foreach($result_list->userEnterLeaveActions as $k => $v){
                    M()->query("insert into el_learn_record(userName,ip,city,enterTime,leaveTime,watchTime,createTime,recordType) values('$v->viewerName','$v->viewerIp','$v->city','$v->enterTime','$v->leaveTime','$v->watchTime',now(),'1')");
                }
            }
        }
        ...
        self::enterLive($ob_start,$ob_end,$uid,$sevenDaysCount);
    ...
    //获取进入直播间的访问人数
    public static function enterLive($ob_start,$ob_end,$uid,$sevenDaysCount){
            ...
            foreach ($result_record as $kk => $vv) {
                ...
                if("OK" == $result_list->result && 0 < $result_list->count){
                    foreach($result_list->userActions as $k => $v){
                        M()->query("insert into el_learn_record(userName,ip,enterTime,createTime,action,recordType,liveId) values('$v->userName','$v->userIp','$v->time',now(),'$result_list->action','2',$vv['id'])");
                    }
                }
    ...
    public function leaveLive($ob_start,$ob_end,$uid,$sevenDaysCount){
        ...
        do{ 
            ...
            foreach ($result_record as $kk => $vv) {
                ...
                if("OK" == $result_list->result && 0 < $result_list->count){
                    foreach($result_list->userActions as $k => $v){
                        M()->query("insert into el_learn_record(userName,ip,leaveTime,createTime,action,recordType,liveId) values('$v->userName','$v->userIp','$v->time',now(),'$result_list->action','2',$vv['id'])");
                    }
                }
    ...
    //     $uid = $_GET['uid'];
    //     $live_record = M("el_live_record")->field("id")->where(['uid='.$uid]);
    //     $stringId = $live_record->findAll();
        ...
        foreach($roomId_array as $key => $val){
            ...
            if("OK" == $result_list->result && 0 < $result_list->count){
                foreach ($result_list->records as $k => $v) {
                    // try{
                        $save_result = M()->query("insert into el_live_record(uid,roomName,roomId,liveId,recordId,startTime,stopTime,saveTime) values(\"$uid\",\"$subject\",\"$roomId\",\"$v->liveId\",\"$v->id\",\"$v->startTime\",\"$v->stopTime\",now())");
                        // if(!$save_result)){
                    //         throw new Exception();
                    //     }    
                    // }catch(Exception $e){
