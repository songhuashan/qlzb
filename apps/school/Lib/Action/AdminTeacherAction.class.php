...
//            $val['DOACTION'].=" | <a href=".U('school/AdminTeacher/style',array('id'=>$val['id'])).";>风采展示</a>";
    ...
    public function article(){
        ...
        if(is_admin($this->mid)){
            ...
            if($tid){
                ...
                $trlist=M("zy_teacher_article article")->join(' INNER JOIN (SELECT id as t_id FROM '.C('DB_PREFIX').'zy_teacher  WHERE mhm_id IN ('. (string) is_school($this->mid) .')) AS t ON t.t_id = article.tid')->where($map)->order("ctime DESC")->findPage(20);
            }
        }
        foreach($trlist['data'] as &$val){
    ...
    public function details(){
        ...
        if(is_admin($this->mid)){
            ...
            if($tid){
               ...
               $trlist = M("zy_teacher_details d")->join(' INNER JOIN (SELECT id as t_id  FROM ' . C('DB_PREFIX') . 'zy_teacher  WHERE mhm_id IN (' . (string)is_school($this->mid) . ')) AS t ON t.t_id = d.tid')->where($map)->order("ctime DESC")->findPage(20);
            }
        }
    ...
    public function style(){
        ...
        if(is_admin($this->mid)){
            ...
            if($tid){
                ...
                $trlist = M("zy_teacher_photos p")->join(' INNER JOIN (SELECT id as t_id FROM ' . C('DB_PREFIX') . 'zy_teacher  WHERE mhm_id IN (' . (string)is_school($this->mid) . ')) AS t ON t.t_id = p.tid')->where($map)->order("ctime DESC")->findPage(20);
            }
        }
        foreach($trlist['data'] as &$val){
            $val['name']    = M('zy_teacher')->where('id='.$val['tid'])->getField('name');
    ...
    public function disable(){
        ...
        if(is_admin($this->mid)){
            ...
            $trlist=M("zy_teacher_article a")->join(' INNER JOIN (SELECT id as t_id FROM '.C('DB_PREFIX').'zy_teacher WHERE mhm_id IN ('. (string) is_school($this->mid) .')) AS t ON t.t_id = a.tid')->where($map)->order("ctime DESC")->findPage(20);
        }
    ...
    public function doAuthenticate(){
        ...
        if(!is_admin($this->mid)){
            ...
            if(is_school($this->mid) != $mhm_id){$this->error('请选择本机构下的用户');}
        }
    ...
    public function doAddTeacher(){
        ...
        $data = array(
            ...
            'title'=>t($_POST['title']),
            'mhm_id'=>intval(is_school($this->mid)),
            'ctime'=>time(),
    ...
    public function courseShow(){
        $this->pageTab[] = array('title'=>'课程展示','tabHash'=>'courseShow','url'=>U('school/AdminTeacher/courseShow'));
        $id=intval($_GET['id']);
