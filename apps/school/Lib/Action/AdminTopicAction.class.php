...
class AdminTopicAction extends AdministratorAction {
	...
	public function index() {
        ...
        $this->pageButton[] = array ('title' => L ( 'PUBLIC_ADD' ), 'onclick' => "javascript:window.location.href='".U('school/AdminTopic/newZixun', array('tabHash' => 'newZixun'))."'");
		...
		foreach ( $list ['data'] as &$v ) {
			...
			$v['action'] = '<a href="'.U('school/AdminTopic/editor', array('tabHash' => 'editor','id'=>$v['id'])).'">编辑</a>';
            if($v['re'] == 1) {
	...
	public function editor() {
		...
		$id = $_GET ['id'];
		if (! $id) {
			$this->assign ( 'jumpUrl', U ( 'school/AdminTopic/index' ) );
			$this->error ( '参数错误' );
		} else {
            // 检测资讯是否属于机构
            if(model('Topics')->where(['id'=>$id,'mhm_id'=>$this->school_id])->count() < 1){
                $this->error ( '无权编辑' );exit;
            }
			...
			$this->pageTab [] = array (
					'title' => '编辑',
					'tabHash' => 'editor',
					'url' => U ( 'school/AdminTopic/editor',['tabHash' => 'editor','id'=>$v['id']])
			);
			...
			$this->opt ['re'] [1] = '是';
			$this->opt ['re'] [0] = '否';
			$this->savePostUrl = U ( 'school/AdminTopic/doeditor', array (
					'id' => $id 
			) );
	...
	public function newZixun() {
		...
		$this->pageTab [] = array ('title' => '添加资讯','tabHash' => 'newZixun','url' => U ( 'school/AdminTopic/newZixun' ) );
		
		$this->pageKeyList = array ('title','desc','text','image','cate','re','from','recount');
		$this->opt ['cate'] = model ( 'Topics' )->getAdmincate (0);
		$this->opt ['re'] [1] = '是';
		$this->opt ['re'] [0] = '否';
		$this->savePostUrl = U ( 'school/AdminTopic/donewZixun' );
		$this->displayConfig ();
	}
	public function donewZixun() {
		if (! $_POST ['title']) {
            ...
            $ary ['mhm_id'] = intval ( $this->school_id );
