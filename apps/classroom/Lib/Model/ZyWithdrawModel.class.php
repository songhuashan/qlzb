...
class ZyWithdrawModel extends Model {
     ...
     */
    public function apply($uid, $wnum, $bcard_id,$wtype){
        //添加提现记录
        $id = $this->add(array(
            'uid'      => $uid,
            'wnum'     => $wnum,
     ...
     */
    public function getUnfinished($uid,$wtype){
        return $this->where("uid=$uid AND wtype=$wtype AND (status=0 OR status=1)")->select();
    }
