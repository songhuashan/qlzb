...
//            $value['DOACTION'] .= ' | <a href="'.U('classroom/AdminSplit/learn',array('uid'=>$value['uid'],'tabHash'=>'learn')).'">TA的学习记录</a>';
    ...
    public function splitVideo()
    {
        ...
        $this->pageKeyList = array('id', 'uid', 'vid', 'video_title', 'sum',
            'platform_sum','sid','school_sum','mount_school_id','mount_school_sum','st_id','school_teacher_sum','share_id','share_sum',
            'ctime', 'ltime', 'note', 'mhm_id');
        $this->pageTitle['splitVideo'] = '点播分成';
        //按钮
        $this->pageButton[] = array('title' => '搜索', 'onclick' => "admin.fold('search_form')");

        //搜索项 搜索条件：课程ID、课程名称、课程价格、平台、平台机构、课程机构、购买机构、分享者、日期筛选
        $this->searchKey              = array('uid', 'vid', 'video_title', 'sum', 'pid', 'sid', 'mount_school_id', 'share_id', ['ltime1', 'ltime2']);
    ...
    {
        // 页面具有的字段，可以移动到配置文件中！
        $this->pageKeyList             = array('id', 'uid', 'aid', 'video_title', 'sum',
            'platform_sum','sid','school_sum','share_id','share_sum',
            'ctime', 'ltime', 'note', 'mhm_id');
        ...
        //搜索项 搜索条件：课程ID、课程名称、课程价格、平台、平台机构、课程机构、购买机构、分享者、日期筛选
        $this->searchKey = array('uid', 'aid', 'video_title', 'sum', 'pid', 'sid', 'mount_school_id', 'share_id', ['ltime1', 'ltime2']);
    ...
    {
        // 页面具有的字段，可以移动到配置文件中！
        $this->pageKeyList            = array('id', 'uid', 'lid', 'video_title', 'sum',
            'platform_sum','sid','school_sum','mount_school_id','mount_school_sum','st_id','school_teacher_sum','share_id','share_sum',
            'ctime', 'ltime', 'note', 'mhm_id');
    ...
    public function splitLineClass()
    {
        // 页面具有的字段，可以移动到配置文件中！
        $this->pageKeyList                 = array('id', 'uid', 'vid', 'video_title', 'sum',
            'platform_sum','sid','school_sum','st_id','school_teacher_sum','share_id','share_sum',
            'ctime', 'ltime', 'note', 'mhm_id');
    ...
    public function splitExport()
    {
        ...
        $xlsCell = [
            ...
            ['platform_sum', '平台分成'],
            ['oschool_uname', '购买机构'],
            ['ouats_ouschool_sum', '购买机构金额'],
            ['sid_uname', "{$sheet_title}所属机构"],
            ['school_sum', "{$sheet_title}所属机构分成"],
            ['mount_school_uname', '挂载机构'],
            ['mount_school_sum', '挂载机构金额'],
            ['share_uname', '分享者'],
            ['share_sum', '分享者分成'],
            ['ctime', '生成时间'],
            ['ltime', '最后操作时间'],
        ...
        $sum                = number_format(array_sum(array_column($listData, 'sum')), 2);
        $platform_sum       = number_format(array_sum(array_column($listData, 'platform_sum')), 2);
        $ouats_ouschool_sum = number_format(array_sum(array_column($listData, 'ouats_ouschool_sum')), 2);
        $school_sum         = number_format(array_sum(array_column($listData, 'school_sum')), 2);
        $mount_school_sum   = number_format(array_sum(array_column($listData, 'mount_school_sum')), 2);
        ...
        array_push($xlsData, [
            ...
            'oschool_uname'      => null,
            'ouats_ouschool_sum' => '￥' . $ouats_ouschool_sum,
            'sid_uname'          => null,
            'school_sum'         => '￥' . $school_sum,
            'mount_school_uname' => null,
            'mount_school_sum'   => '￥' . $mount_school_sum,
            'share_uname'        => null,
            'share_sum'          => '￥' . $share_sum,
            'ctime'              => null,
            'ltime'              => null,
        ...
        model('Excel')->export("{$sheet_title}分成明细导出", $xlsCell, $xlsData);
