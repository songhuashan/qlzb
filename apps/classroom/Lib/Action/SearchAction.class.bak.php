...
//		$aRes = array_slice($rs,$offset,10);
		foreach($rs as $key=> $vo){
			if($vo['type'] == '1'){
				...
				$resT['data'][$key] = M('album')->where('id = '.$vo['id'])->field('album_title as title,album_category as category,album_intro as intro,album_collect_count as collect,uid,id,album_score as score,middle_ids')->find();
				$resT['data'][$key]['collection'] =  M('zy_collection')->where('source_id='.$vo['id'].' and source_table_name="album"')->count();
