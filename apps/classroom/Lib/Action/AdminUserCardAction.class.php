                ...
                if($type == 5){
                    ...
                    if($coupon == 3){
                        ...
                        foreach($list as $key=>$val){
                            $new_where = $where." AND id = ".$val['video_id'];
                            $list[$key]['code'] =  D('Album','classroom')->where($new_where)->getField('album_title');
                        }
                    }else{
                        $where = "is_del=0 AND is_mount = 1 AND is_activity IN (1,5,6,7) AND uctime>$time AND listingtime<$time";
                        foreach($list as $key=>$val){
                            $new_where = $where." AND id = ".$val['video_id'];
                            $list[$key]['code'] =  D('ZyVideo','classroom')->where($new_where)->getField('video_title');
                        }
                    }
                }else{
