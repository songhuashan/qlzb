	...
	public function memberaction() {
		...
		switch ($_POST['op'])
		{
			...
			case 'allow':   // 批准成为会员
				...
				if ($res) {
                    $group_name = D('Group')->where('id='.$this->gid)->getField('name');
					$message['title']   = "申请加入群组 {$group_name}成功";
					$message['content'] = "你申请加入群组“{$group_name}”的请求被批准,你已经成为该群组的成员";
					$message['to']      = $this->uid;
					$res = model('Message')->postMessage($message,  $this->mid);
