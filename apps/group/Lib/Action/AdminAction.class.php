...
class AdminAction extends AdministratorAction {
	...
	{//为使搜索条件在分页时也有效，将搜索条件记录到SESSION中
		...
		if ('group' == $type) {
			...
			$replymap['table'] = 'group_topic';
			$replymap['to_uid'] = ['neq',0];
			foreach($list['data'] as &$value) {
				$replymap['row_id'] = $value['id'];
				$value['replycount'] = model('Comment')->where($replymap)->count();
			}
		} else if ('post' == $type) {
			// 回帖
   			//$list = D('Post')->getPostList(1, $condition, null, $order, $limit, 0);
            $postmap = ['table'=>'group_topic','to_uid'=>['neq',0],'is_del'=>0];
            $list = model('Comment')->where($postmap)->findPage($limit);
            foreach($list['data'] as &$value) {
                $value['id'] = $value['comment_id'];
                $topic = D('Topic')->where(['id'=>$value['row_id']])->field('gid,title')->find();
                $value['gid'] = $topic['gid'];
	    	 ...
	    	 * 缓存帖子标题
	    	 */
	    	$rids = getSubByKey($list['data'], 'row_id');
	    	$ttitles = D('Topic')->field('id,title')->where('id IN (' . implode(',', $rids) . ')')->findAll();
